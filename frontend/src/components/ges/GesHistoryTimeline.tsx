...
export function GesHistoryTimeline({ gesId }: GesHistoryTimelineProps) {
  ...
  const tmertList = data.tmertReports || [];

  const allPrescriptions = [
    ...(tech?.prescriptions || []),
    ...quantList.flatMap((q: any) => q.prescriptions || []),
    ...tmertList.flatMap((t: any) => t.prescriptions || []),
  ];
          ...
          <div className="relative pl-5 mt-2">
            ...
            {tech ? (
              <div className="relative mb-6 pl-4">
                ...
                >
                  <div className="mt-0.5 shrink-0">
                    {expandedQuali ? <ChevronDown className="h-4 w-4 text-slate-600" /> : <ChevronRight className="h-4 w-4 text-slate-600" />}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-slate-500 break-words leading-tight">
                      {new Date(tech.reportDate).toLocaleDateString()} · Folio {tech.reportNumber}
                    </p>
                  </div>
                ...
                {expandedQuali && (
                  <div className="mt-2 pl-6 animate-in fade-in duration-300 space-y-2">
                    <div className="flex items-center gap-2 text-xs text-slate-600">
                      ...
                      {tech.pdfUrl && (
                        <Button asChild size="sm" variant="outline" className="h-6 px-2 text-[11px] ml-1">
                          <a href={tech.pdfUrl} target="_blank" rel="noreferrer">Ver PDF</a>
                        </Button>
                      )}
                    </div>
            ...
            {quantList.map((q: any, idx: number) => {
              ...
              return (
                <div key={q.id} className="relative mb-6 pl-4 last:mb-0">
                  ...
                  <button
                    onClick={() => setExpandedQuant({ ...expandedQuant, [q.id]: !isOpen })}
                    className="flex items-start gap-2 w-full text-left"
                  >
                    <div className="mt-0.5 shrink-0">
                  ...
                  {isOpen && (
                    <div className="mt-2 pl-6 animate-in fade-in duration-300 space-y-2">
                      <div className="flex items-center gap-2 text-xs text-slate-600">
                        ...
                        {q.pdfUrl && (
                          <Button asChild size="sm" variant="outline" className="h-6 px-2 text-[11px] ml-1">
                            <a href={q.pdfUrl} target="_blank" rel="noreferrer">Ver PDF</a>
                          </Button>
                        )}
                      </div>
            ...
            {tmertList.map((t: any) => {
                ...
                return (
                  <div key={t.id} className="relative mb-6 pl-4 last:mb-0">
                    ...
                    <button
                      onClick={() => setExpandedTmert({ ...expandedTmert, [t.id]: !isOpen })}
                      className="flex items-start gap-2 w-full text-left"
                    >
                      <div className="mt-0.5 shrink-0">
                        {isOpen ? <ChevronDown className="h-4 w-4 text-slate-600" /> : <ChevronRight className="h-4 w-4 text-slate-600" />}
                    ...
                    {isOpen && (
                      <div className="mt-2 pl-6 animate-in fade-in duration-300 space-y-2">
                        <div className="flex items-center gap-2 text-xs text-slate-600">
                          ...
                          {t.pdfUrl && (
                            <Button asChild size="sm" variant="outline" className="h-6 px-2 text-[11px] ml-1">
                              <a href={t.pdfUrl} target="_blank" rel="noreferrer">Ver PDF</a>
                            </Button>
                          )}
                        </div>
