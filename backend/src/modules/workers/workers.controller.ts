...
import { findAllWorkers, importWorkersDb, findWorkerByRut, getWorkerById, updateWorker, deleteWorker, analyzeJobChange, transferWorker, createWorkerDb } from './workers.service';
...
export const update = async (req: Request, res: Response) => {
    try { res.json(await updateWorker(req.params.id, req.body)); } catch (e) { res.status(500).json({ error: 'Error actualizar' }); }
};
